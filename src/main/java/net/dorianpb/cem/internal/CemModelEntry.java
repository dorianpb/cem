...
class CemModelEntry{
	...
	private CemModelEntry(String part, JpmFile data, Model in, float[] debugpos, float[] parents, int gen, float scale, int textureWidth, int textureHeight){
		...
		this.translates = new float[]{(data.getTranslate().get(0).floatValue()) * (this.invertAxis[0]? -1 : 1) + debugpos[0],
	...
	CemModelEntry(JemModel file, JemFile jemFile){
		this(file.getPart(),
		     ...
		     new float[]{0, 24, 0},
		     0,
		     file.getScale().floatValue(),
		     jemFile.getTextureSize().get(0).intValue(),
		     jemFile.getTextureSize().get(1).intValue()
		    );
	}
	...
	private void initmodel(JpmFile data, float[] debugpos, float[] parents, int gen, float scale){
		...
		if(data.getBoxes() != null){
			for(JpmBox box : data.getBoxes()){
				...
				this.model.addCemCuboid(((box.getCoordinates().get(0).floatValue() + ((gen == 0)? translate[0] : 0)) * ((this.invertAxis[0])? -1 : 1)) -
	...
	static class CemModelPart extends ModelPart{
		...
		float getRotation(char axis){
			switch(axis){
				case 'x':
					return this.pitch + this.rotation[0];
				case 'y':
					return this.yaw + this.rotation[1];
				case 'z':
					return this.roll + this.rotation[2];
				default:
					throw new IllegalStateException("Unknown axis \"" + axis + "\"");
			}
		}
